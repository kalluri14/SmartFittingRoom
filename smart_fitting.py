...
def keypoint():
    ...
    while True:
        ...
        cv2.putText(img, str(int), (70, 50), cv2.FONT_HERSHEY_PLAIN, 3,
                    (255, 0, 0), 3)
...
def depth():
    ...
    while True:
        ...
        if faces:
            ...
            w, _ = detector.findDistance(pointRight, pointLeft)
            W = 6.3
            ...
            if d< 110:
               st.write('move away')
                ...
                st.write('you are in right position')
...
def main():
    st.title("SMART FITTING ROOM")
    activities = ["VIRTUALMOUSE","DEPTH","KEY-POINT", "MEASUREMENT","SIZE-CHART"]
    choice = st.sidebar.selectbox("Select Activity", activities)

    if choice == 'KEY-POINT':
        st.subheader("Please stand in front of the camera")
        summary_choice = st.selectbox("choose an option", ["KEYPOINT"])
        if st.button("Result"):
            if summary_choice == 'KEYPOINT':
                summary_result = keypoint()
            st.write(summary_result)

    if choice == 'DEPTH':
        st.subheader("Please stand in front of the camera")
        summary_choice = st.selectbox("choose an option", ["DEPTH"])
        if st.button("Depthresult"):
            if summary_choice == 'DEPTH':
                summary_result = depth()
            st.write(summary_result)

    if choice == 'VIRTUALMOUSE':
        #st.subheader("Using Open CV")
        summary_choice = st.selectbox("choose an option", ["VIRTUALMOUSE"])
        if st.button("Activate"):
            if summary_choice == 'VIRTUALMOUSE':
                summary_result = virtual_mouse()
            st.write(summary_result)

    if choice == 'MEASUREMENT':
        st.subheader(" Welcome to Measurements Segment")
        summary_choice = st.selectbox("choose an option",["MEASUREMENT"])
        if st.button("Measure"):
            if summary_choice == 'MEASUREMENT':
                summary_result = measurement()
            st.write(summary_result)

    if choice == 'SIZE-CHART':
        from PIL import Image
        st.subheader(" SIZE-CHART")
        image = Image.open('sizechart.jpg')
        st.image(image)
